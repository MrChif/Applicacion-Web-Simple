...
<br>
<div class="col-md-7">
  <table class="table">  <!-- Tabla de datos, que se encuentran en la base de datos -->
    <thead>
      <tr>
    ...
    <tbody>
    <?php foreach($listaprof as $prof){ ?>
      <tr class="">
        <td><?php  echo $prof['Id_prof']?></td>
        <td><?php  echo $prof['Nombre']?></td>
        <td><?php  echo $prof['Apellido_p']?></td>
        <td><?php  echo $prof['Apellido_m']?></td>
        <td>
          <form action='' method='post'>
            <input type="hidden" name="id" id="id" value="<?php  echo $prof['Id_prof']?>" />
